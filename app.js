...
function screen(id){$$('.screen').forEach(x=>x.classList.toggle('active',x.id===id));$$('.bottom-nav button').forEach(x=>x.classList.toggle('active',x.dataset.screen===id));window.scrollTo({top:0,behavior:'smooth'});if(id==='rankings')renderRankings();if(id==='players'){renderPlayers();loadHistoricalPlayerDirectory().catch(()=>{});}if(id==='admin')renderAdmin();if(id==='stats'){startLiveStats()}if(id==='teamstats')renderTeamStats();if(id==='news')renderNews();if(id==='predictor')renderPredictor();}
...
function openPlayerStats(name){
  ...
  $('#playerStatsBody').innerHTML=`<div class="player-stats-grid">
    ...
    <div><b>${d.tries}</b><small>Drieë</small></div><div><b>${d.from}${d.from!=='—'?' – '+d.to:''}</b><small>Internasionale / Test-loopbaan</small></div>
...
function renderPlayers(){
  ...
  const allowedCountries=['🇿🇦','🇳🇿','🇮🇪','🇫🇷','🏴ENG','🏴SCO','🇦🇺','🇦🇷','🇫🇯','🏴WAL','🇼🇸'];const list=directoryPlayers.filter(p=>allowedCountries.includes(playerCountryKey(p))&&(pos==='all'||p.pos===pos)&&(!q||p.name.toLowerCase().includes(q)));
...
const HISTORICAL_PLAYER_SOURCES=[
  ...
  ['Australia','🇦🇺','Australia.csv'],['Argentina','🇦🇷','Argentina.csv'],['Fiji','🇫🇯','Fiji.csv'],['Wales','🏴WAL','Wales.csv'],['Samoa','🇼🇸','Samoa.csv']
];
...
async function loadHistoricalPlayerDirectory(){
  ...
  const merged=[...directoryPlayers.filter(p=>HISTORICAL_PLAYER_SOURCES.some(x=>x[1]===p.country)||playerCountryKey(p).match(/ENG|WAL|SCO/))];
  await Promise.all(HISTORICAL_PLAYER_SOURCES.map(async ([country,flag,file])=>{
    try{
      ...
      const ni=head.indexOf('name'),di=head.indexOf('debut'),pi=head.indexOf('position'),mi=head.indexOf('matches'),pts=head.indexOf('points'),ti=head.indexOf('tries');
      if(ni<0)return;
      for(const line of lines.slice(1)){
        const row=csvFields(line),name=(row[ni]||'').trim(),year=debutYear(row[di]);
        if(!name||year<1900||year>2026)continue;
        const matches=Number(String(row[mi]||'').replace(/[^0-9.-]/g,''))||0;
        // Only genuine capped/test players belong in this directory.
        if(matches<1)continue;
        const pos=positionNumber(row[pi]);
        const p={name,country:flag,pos};
        merged.push(p);
        const old=PLAYER_STATS[name]||{};
        PLAYER_STATS[name]={
          ...old,
          h:old.h||'—',w:old.w||'—',
          caps:matches,
          points:Number(String(row[pts]||'').replace(/[^0-9.-]/g,''))||0,
          tries:Number(String(row[ti]||'').replace(/[^0-9.-]/g,''))||0,
          from:old.from||year,
          to:old.to||'—'
        };
      }
  ...
  // Keep only the requested Top 10 nations + Samoa, with country flags normalized.
  directoryPlayers=uniquePlayers(merged).filter(p=>{
    const k=playerCountryKey(p);
    return ['🇿🇦','🇳🇿','🇮🇪','🇫🇷','🏴ENG','🏴SCO','🇦🇺','🇦🇷','🇫🇯','🏴WAL','🇼🇸'].includes(k);
  });
  renderPlayers();
  hydratePhotos($('#playerGrid'));
}
...
loadLocalPlayers();loadVoteCounts().then(()=>renderPositions());progress();renderRankings();renderPlayers();renderAdmin();renderTeamStats();renderNews();
